  ...
  interface PackedItemData {
    name: string;
    position: any; // can be array [x,y,z] or object {x,y,z}
    dimensions: {
      length: number;
      width: number;
      height: number;
  ...
  function BoxUI() {
    ...
    const closeLeftSidebar = () => setLeftSideBar(false);
    ...
    const closeRightSidebar = () => setRightSideBar(false);
    ...
    useEffect(() => {
      ...
      const volumes = boxDimensions.map((box) => {
        ...
        return isNaN(boxVolume) ? 0 : Number(boxVolume.toFixed(6));
      });
      ...
      <div className="h-screen w-full relative text-black overflow-x-hidden overflow-y-hidden">
        <div className="flex justify-between items-center md:hidden w-full relative top-0 left-0 bg-white py-5 px-5 z-50">
          ...
          <button
            ...
            onClick={() => {
              if (!results) return;
              const blob = new Blob([JSON.stringify({ results, numContainers }, null, 2)], {
              ...
              <div key={index} className="rounded-md py-1 bg-white mb-2">
                <div className="flex justify-between items-center px-1">
                  ...
                  <p className="text-sm text-gray-400">
                    {box.length || "L"} x {box.width || "W"} x {box.height || "H"} {box.unit}
                  </p>
                  {box.collapsed ? (
                    <MdOutlineKeyboardArrowDown
